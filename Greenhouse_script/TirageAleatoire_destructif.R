...
#T101-------------
#Dry treatment D3 Recovery
#Retirer individus déjà selectionnés en T0, T21, T27, T51, T57, T71-----
...
tirage_T101  <- tirage_T101  %>% add_column (RWC_FW = NA, RWC_SW = NA, RWC_DW = NA, LT1 = NA, LT2 = NA, LT3 = NA, LT4 = NA, LT5 = NA, MeanLT = NA, ScanLeaf = NA, ScanRoots = NA)

tirage_T101_destructif<- tirage_T101  %>% relocate(Remarques, .after = ScanRoots)

library(rio)
export(tirage_T101_destructif, "tirage_T101 _destructif.csv")
...
T101_epefal_destruction <-sample(epefal_C0$UniqueCode, 3, replace = FALSE)
#results: 494; 296;505

#Virola surinamensis
virsur_C0 <- ControlGroup[ControlGroup$Species== "Vir.sur",]
T101_virsur_destruction <-sample(virsur_C0$UniqueCode, 3, replace = FALSE)
#1results: 186;319;934
